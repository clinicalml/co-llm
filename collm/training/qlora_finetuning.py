...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="If passed, we assume the training file is already tokenized.",
    )
    parser.add_argument(
        "--config_name",
        type=str,
...
def main():
    ...
    if args.dataset_name is not None:
        if os.path.isdir(args.dataset_name) and os.path.exists(args.dataset_name):
            raw_datasets = datasets.load_from_disk(args.dataset_name)
            columns_to_remove = ["reference_log_probs", "reference_max_prob"]
            columns_to_remove = [col for col in columns_to_remove if col in raw_datasets.column_names]
    ...
    if not args.skip_data_preprocessing:
        ...
        with accelerator.main_process_first():
            train_dataset = raw_datasets.filter(lambda example: (example["labels"] != -100).any())
