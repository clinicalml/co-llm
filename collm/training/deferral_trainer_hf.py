...
def main():
    ...
    if model_args.no_deferral_initialization_search:
        logger.info("No deferral token initialization search requested, skipping.")
        gperp = None
    else:
        with training_args.main_process_first(desc="Search for deferral token initialization"):
            if os.path.isfile(f"{training_args.output_dir}/gperp_init.bin"):
                logger.info(
                    "Loading deferral token initialization from %s",
                    f"{training_args.output_dir}/gperp_init.bin",
                )
                gperp = torch.load(f"{training_args.output_dir}/gperp_init.bin")
            else:
                ...
                search_deferral_initialization = deferral_training_tools.ALL_INITIALIZATION_SEARCH[
                    model_args.deferral_initialization_search_version
                ]
                gperp = search_deferral_initialization(
                    model,
                    data_module,
                    batch_size=8,
                    deferral_weight=model_args.deferral_initialization_weight_balance,
                    max_steps=None,
                )
                os.makedirs(training_args.output_dir, exist_ok=True)
                torch.save(gperp, f"{training_args.output_dir}/gperp_init.bin")
                sys.exit()
        # add deferral token and initialization

    num_added_tokens = tokenizer.add_special_tokens(dict(additional_special_tokens=[DEFAULT_DEFERRAL_TOKEN]))
    assert num_added_tokens == 1, "Should only add one special token - the deferral token."

    deferral_training_tools.smart_tokenizer_and_embedding_resize_for_def_token(tokenizer, model, gperp)
    ...
    DeferralTrainer = deferral_training_tools.ALL_DEFERRAL_TRAINERS[model_args.deferral_trainer_version]
    trainer = DeferralTrainer(
        model=model,
        tokenizer=tokenizer,
        args=training_args,
        **data_module,
